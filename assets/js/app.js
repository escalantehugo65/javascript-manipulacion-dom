...
const renderizarCatalogo = (listaPersonajes) => {
  ...
  listaPersonajes.forEach((personaje) => {
    const { id, nombre,imagen } = personaje;
      ...
      const cardHtml=`<div class="col">
        ...
        `;
    contenedorCards.innerHTML += tarjeta;
  });
};

const eliminarPersonaje = (idSeleccionado) => {
  const indice = personajes.findIndex((p) => p.id === idSeleccionado);

  if (indice !== -1) {
    personaje.splice(indice, 1);
...
evento.preventDefault();
    ...
    if (urlImagen===""){
        urlImagen="https://media.istockphoto.com/id/1147544807/es/vector/no-imagen-en-miniatura-gr%C3%A1fico-vectorial.jpg?s=1024x1024&w=is&k=20&c=jMNMEtHs4WCgUd9WCoR2gcKdD0UAOFoTROlutZRWNLE=";
    }
const nuevoPersonaje = {
  id: Date.now(), //crea un id unico en ese mismo momento//
